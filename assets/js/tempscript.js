...
var questionObj = {
  //question object that holds all the parts of questions
  questions: [
    //questions can just be added to by adding on a string to end of array
  ...
  answers: [
    //answers are in a 2d array because multiple answers for 1 questions
    ...
    [
      ...
      `The <footer> section`,
    ], //uses `correct:` so that even if answer has the word `correct` its not flagged as correct answer
    [
    ...
    [
      ...
      `msg("Hello World");`,
    ], //to pull out correct: newStr = substring(7,questionObj.answers[index].length)
...
function startTimer() {
  var timerInterval = setInterval(function () {
    if (gameEnded === true) {
      //test if game ended before anything incase needs to be stopped
      clearInterval(timerInterval); //stop
      return;
    }
    if (timeLeft < 1) {
      //if timer is out under 1 cause wrong answers subtract 10 seconds game ends and timer stops
      clearInterval(timerInterval); //stop
...
function createAnswerElements(currentQuestionIndex) {
  ...
  for (
    ...
    //loop over every answer (for current question) and create a list item on the page based on that content
    ...
    if (
      questionObj.answers[currentQuestionIndex][answerIndex].includes(
        `correct:`
      )
    ) {
      tempStr = questionObj.answers[currentQuestionIndex][
        answerIndex
      ].substring(
        8,
        questionObj.answers[currentQuestionIndex][answerIndex].length
      ); //yoink out the string part that doesnt contain `correct:`
...
function endGame() {
  //needs to be callabled from multiple areas so its a function
...
function checkAnswer(event) {
  if (event.target != answerButtonLst) {
    //if this is just the <ul> dont do anything else we just want the <li> (answers) themselves not parent

    if (!event.target.id.includes("correct")) {
      //check <li> id to see if its tagged as the correct answer
...
function storeScoreAndName() {
  ...
  if (highscoreTextbox.value != `` || highscoreTextbox.value != null) {
    //as long as textbox is not empty or null can continue to try to store this.
    var tempObject = {
      //initialize a object to put in the storage array
    ...
    if (window.localStorage.getItem(`highscores`) == null) {
      //if no data exsists create a new array of objects
      tempArrayOfObjects.push(tempObject); //push current user score and name into our empty array
      window.localStorage.setItem(
        `highscores`,
        JSON.stringify(tempArrayOfObjects)
      ); //send our new array into local storage
    } else {
      //if some data previously was stored and exsist lets add onto it and put the score in the right spot of the leaderboard
      tempArrayOfObjects = JSON.parse(
        window.localStorage.getItem(`highscores`)
      ); //get and parse our array into a usable variable
      ...
      for (let index = 0; index <= tempArrayOfObjects.length; index++) {
        ...
        if (index == tempArrayOfObjects.length) {
          //if we are at the end of the array then just put our new score at the bottom because the new score wasnt higher than any previous
          ...
          //if our new score is higher than the current score throw it in the array so its sorted high to low
      ...
      window.localStorage.setItem(
        `highscores`,
        JSON.stringify(tempArrayOfObjects)
      ); //turn our array of objects into a string and store it in local storage
    }
...
function showHighscores() {
  ...
  if (tempArrayOfObjects != null) {
    //only continue if there was data to use and display stuff on highscore board
    for (let index = 0; index < tempArrayOfObjects.length; index++) {
      //loop over every array element found (highscore entry)
      var newLi = document.createElement(`li`); //create a new <li> to append to our <ol>
      newLi.textContent =
        tempArrayOfObjects[index].names +
        ` - ` +
        tempArrayOfObjects[index].scores; //fill up new <li> with content of stored highscores
      tempOrderedList.appendChild(newLi); //append to parent <ol> (numbered list)
    }
  } else {
    //if there was no data in local storage to show on highscores show error
